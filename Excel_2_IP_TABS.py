...
def read_excel(file_name, sheet_name):
    # Reads an Excel file and returns IPs from column C of the specified sheet
...
def save_output(output, hostname):
    ...
    filename = f'{directory}/{hostname}_{now.year}-{now.month}-{now.day}_TRACK_1.txt'
...
def main():
    ...
    cisco_devices = read_excel('TEST_LAB.xlsx', 'CISCO_TEST')
    router_devices = read_excel('TEST_LAB.xlsx', 'ROUTER')

    # Process devices in CISCO_TEST sheet
    for device_ip in cisco_devices:
        print("Connecting to device (CISCO_TEST): " + device_ip)
        try:
    ...
    # Process devices in ROUTER sheet
    for device_ip in router_devices:
        print("Connecting to device (ROUTER): " + device_ip)
        try:
            connection = connect_to_device(device_ip, 'cisco_ios', username, password)
            print("Entering the enable mode...")
            output = execute_commands(connection, commands_list)
